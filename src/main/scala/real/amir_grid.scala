...
object amir_grid extends App{
  ...
  sw_default("max_util",1)("granularity",16)(
...
object command_gen extends App{
  val pw = new PrintWriter(new File(s"./DFG/command_v4.sh"))
  for(num_row <- 4 to 16; num_col <- 4 to num_row; dfg_idx <- 2000 until 3000){
    pw.write(s"ss_sched ./configs/amir/dev_row${num_row}_col${num_col}.json ./dfgs/amir/dfg$dfg_idx.dfg --verbose\n")
  }
  pw.close()
}