...
import cgra.fabric.Shared_PE_component.instruction.mm_instruction_t
import chisel3._
...
object derived_parameters extends initial_parameters{
  ...
  */
  ...
  def parameter_update(p:shared_pe):Unit={
    // --------------  Component Config ---------
    TIA_HAS_CORE_MONITOR = p.has_debug_monitor
    TIA_HAS_CORE_PERFORMANCE_COUNTERS = p.has_performance_counters
    TIA_HAS_MULTIPLIER = p.has_multiplier
    TIA_SOFTWARE_ROUTER = p.has_software_router
    TIA_SWITCH_ROUTER = p.has_switch_router
    TIA_HAS_SCRATCHPAD = p.has_scratchpad

    // --------------  Knob  --------------------
    TIA_WORD_WIDTH = data_word_width
    TIA_NUM_REGISTERS = p.num_registers
    TIA_SPM_DEPTH = p.spm_depth

    // Memory-mapped instruction representation.
    TIA_MM_INSTRUCTION_WIDTH = p.mm_instruction_width

    // Maximum number of instructions per PE.
    TIA_MAX_NUM_INSTRUCTIONS = p.num_instructions

    Instructions = p.instructions

      // Number of input and output channels.

    TIA_INPUT_CHANNELS = p.input_ports
    TIA_OUTPUT_CHANNELS = p.output_ports
    TIA_NUM_INPUT_CHANNELS = TIA_INPUT_CHANNELS.length
    TIA_NUM_OUTPUT_CHANNELS = TIA_OUTPUT_CHANNELS.length
    // Instruction immediates.
    TIA_IMMEDIATE_WIDTH = p.immediate_width
    ...
    TIA_NUM_PREDICATES = p.num_predicates

    // Tags for inter-PE/ME channels.
    TIA_NUM_TAGS = p.num_tags

    // Maximum number of input channels upon which an instruction can depend.
    TIA_MAX_NUM_INPUT_CHANNELS_TO_CHECK = p.max_num_input_channels_to_check

    // --- Interconnect ---

    // Number of physical planes.
    TIA_NUM_PHYSICAL_PLANES = p.num_physical_planes

    // Generic channel FIFO depth.
    TIA_CHANNEL_BUFFER_FIFO_DEPTH = p.channel_buffer_depth

    // Link buffer FIFO depth.
    TIA_LINK_BUFFER_FIFO_DEPTH = TIA_CHANNEL_BUFFER_FIFO_DEPTH

    // Memory link buffer FIFO depth.
    TIA_MEMORY_LINK_BUFFER_FIFO_DEPTH = p.memory_link_buffer_fifo_depth

    // --- Memory ---

    TIA_NUM_DATA_MEMORY_WORDS = p.num_data_memory_words

    // --- Scratchpad ---
    TIA_NUM_SCRATCHPAD_WORDS_IF_ENABLED = p.num_scratchpad_words_if_enabled
