...
class link_if_t(tag_width:Int,word_width:Int) extends Bundle {
  val packet = new packet_t(tag_width,word_width)
  val valid = Bool()
  val ready = Bool()
  override def cloneType =
    new link_if_t(tag_width,word_width).asInstanceOf[this.type]
}
