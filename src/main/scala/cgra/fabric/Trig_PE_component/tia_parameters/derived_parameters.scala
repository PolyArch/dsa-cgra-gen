...
object derived_parameters extends initial_parameters{
  ...
  */
  ...
  def parameter_update(p:mutable.Map[String,Any]):Unit={
    // --------------  Component Config ---------
    TIA_HAS_CORE_MONITOR = p("has_debug_monitor").asInstanceOf[Boolean]
    TIA_HAS_CORE_PERFORMANCE_COUNTERS = p("has_performance_counters").asInstanceOf[Boolean]
    TIA_HAS_MULTIPLIER = p("has_multiplier").asInstanceOf[Boolean]
    TIA_SOFTWARE_ROUTER = p("has_software_router").asInstanceOf[Boolean]
    TIA_SWITCH_ROUTER = p("has_switch_router").asInstanceOf[Boolean]
    TIA_HAS_SCRATCHPAD = p("has_scratchpad").asInstanceOf[Boolean]

    // --------------  Knob  --------------------
    TIA_WORD_WIDTH = data_word_width
    TIA_NUM_REGISTERS = p("num_registers").asInstanceOf[Int]
    TIA_SPM_DEPTH = p("spm_depth").asInstanceOf[Int]

    // Memory-mapped instruction representation.
    TIA_MM_INSTRUCTION_WIDTH = p("mm_instruction_width").asInstanceOf[Int]
    // Maximum number of instructions per PE.
    TIA_MAX_NUM_INSTRUCTIONS = p("num_instructions").asInstanceOf[Int]
    Instructions = p("instructions").asInstanceOf[List[String]]

      // Number of input and output channels.

    TIA_INPUT_CHANNELS = p("input_ports").asInstanceOf[List[String]]
    TIA_OUTPUT_CHANNELS = p("output_ports").asInstanceOf[List[String]]
    TIA_NUM_INPUT_CHANNELS = TIA_INPUT_CHANNELS.length
    TIA_NUM_OUTPUT_CHANNELS = TIA_OUTPUT_CHANNELS.length
    // Instruction immediates.
    TIA_IMMEDIATE_WIDTH = p("immediate_width").asInstanceOf[Int]
    ...
    TIA_NUM_PREDICATES = p("num_predicates").asInstanceOf[Int]

    // Tags for inter-PE/ME channels.
    TIA_NUM_TAGS = p("num_tags").asInstanceOf[Int]

    // Maximum number of input channels upon which an instruction can depend.
    TIA_MAX_NUM_INPUT_CHANNELS_TO_CHECK = p("max_num_input_channels_to_check").asInstanceOf[Int]

    // --- Interconnect ---

    // Number of physical planes.
    TIA_NUM_PHYSICAL_PLANES = p("num_physical_planes").asInstanceOf[Int]

    // Generic channel FIFO depth.
    TIA_CHANNEL_BUFFER_FIFO_DEPTH = p("channel_buffer_depth").asInstanceOf[Int]

    // Link buffer FIFO depth.
    TIA_LINK_BUFFER_FIFO_DEPTH = TIA_CHANNEL_BUFFER_FIFO_DEPTH

    // Memory link buffer FIFO depth.
    TIA_MEMORY_LINK_BUFFER_FIFO_DEPTH = p("memory_link_buffer_fifo_depth").asInstanceOf[Int]

    // --- Memory ---

    TIA_NUM_DATA_MEMORY_WORDS = p("num_data_memory_words").asInstanceOf[Int]

    // --- Scratchpad ---
    TIA_NUM_SCRATCHPAD_WORDS_IF_ENABLED = p("num_scratchpad_words_if_enabled").asInstanceOf[Int]
