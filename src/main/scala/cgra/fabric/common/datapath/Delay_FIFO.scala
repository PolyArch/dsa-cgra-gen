...
class Delay_FIFO_Hw(p:Delay_FIFO) extends Module{
  ...
  if (p.max_delay > 0){
    if(p.protocol.contains("Ready")){
      ...
      if(p.protocol.contains("Config")){
        val queue_in_config : DecoupledIO[Bool] = Wire(DecoupledIO(Bool()))
        queue_in_config.bits := io.in.config
        queue_in_config.valid := io.in.valid
        queue_in_config.ready <> io.in.ready
        val queue_out_config = Queue(queue_in_config,p.max_delay)
        io.out.config := queue_out_config.bits
        io.out.valid := queue_out_config.valid
        io.out.ready <> queue_out_config.ready
      }
    }else if (p.protocol.contains("Data")){
      // Dalay Pipe
      val head = RegInit(0.U((1 max log2Ceil(p.max_delay)).W))
      val tail = RegInit(0.U((1 max log2Ceil(p.max_delay)).W))
      head := head + 1.U
      ...
      if(p.protocol.contains("Config")){
        val fifo_config = Reg(Vec(p.max_delay,Bool()))
        io.out.config := fifo_config(head)
        fifo_config(tail) := io.in.config
      }
    }
  }else{
