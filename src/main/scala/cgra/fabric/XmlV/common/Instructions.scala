...
object Instructions {
  ...
  val TIA_OP_CTZ = 38
  val TIA_OP_HALT = 39
  //val basic_insts_hw = VecInit((TIA_OP_NOP to TIA_OP_HALT).map(x=>x.U))

  // Scratchpad extensions.
  val TIA_OP_LSW = 40
  val TIA_OP_SSW = 41
  //val scratchpad_insts_hw = VecInit((TIA_OP_LSW to TIA_OP_SSW).map(x=>x.U))
  ...
  //val load_store_inst_hw = VecInit((TIA_OP_RLW to TIA_OP_SW).map(x=>x.U))
  ...
  //val integer_mult_insts_hw = VecInit((TIA_OP_LMUL to TIA_OP_MAC).map(x=>x.U))
  ...
  //val float_point_insts_hw = VecInit((TIA_OP_ITF to TIA_OP_FMAC).map(x=>x.U))

  // Define Instruction Set
  val signed_insts_list = List(TIA_OP_ASR, TIA_OP_SGT, TIA_OP_SLT, TIA_OP_SGE, TIA_OP_SLE)
  //val signed_insts_hw = VecInit(signed_insts_list.map(x=>x.U))
  ...
  //val unsigned_insts_hw = VecInit(unsigned_insts_list.map(x=>x.U))

  val multi_insts_list = List(TIA_OP_LMUL,TIA_OP_SHMUL,TIA_OP_UHMUL,TIA_OP_MAC)
  //val multi_insts_hw = VecInit(multi_insts_list.map(x=>x.U))
