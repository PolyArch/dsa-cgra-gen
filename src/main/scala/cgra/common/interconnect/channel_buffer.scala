...
class output_channel_buffer(bufferDepth :Int) extends Module{
  ...
  channel_buffer.sender.dequeue := io.out.req
  io.out.ack := !channel_buffer.sender.empty
...
class input_channel_buffer(bufferDepth :Int) extends Module{
  ...
  io.in.ack := !channel_buffer.receiver.full
...
object inputChannelDriver extends App {
  chisel3.Driver.execute(args, () => new input_channel_buffer(15))
}
