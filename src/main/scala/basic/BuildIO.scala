...
trait BuildIO {
  ...
  def get_Port_fromXML(node:NodeSeq) : ListBuffer[Port] = {
    ...
    for (pe <- port_entity_list){
      val iotype = pe.get("IO_Type").toString
  ...
  def get_Parameters_fromXML(node:NodeSeq):Map[String,PValue] = {
    var Parameters : Map[String,PValue] = Map[String,PValue]()
