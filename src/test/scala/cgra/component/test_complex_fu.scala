...
object test_complex_fu extends App{
  ...
  private val num_tail_zero = num_bits -
    num_config_bit - num_curr_util_bit - num_id_bit -
    ...
    dut => new PeekPokeTester[complex_fu](dut) {
      ...
      def feed_random_values : Boolean = {
        val input_values = for(input_idx <- 0 until num_input) yield {
          if(input_idx == config_in_port_idx){
            if(nextInt(100) < numeric_conf_ratio){
              BigInt(57257)//generate_random_numeric_value
            }else{
  ...
  def generate_random_numeric_value: BigInt = {
    val nxt_val : BigInt = 9527//BigInt(data_width, scala.util.Random)
