...
report_dir = "/home/sihao/ss-stack/ss-cgra-gen/syn/Reports_dedicated_router/Reports/"
output_dir = "/home/sihao/ss-stack/ss-cgra-gen/syn/Reports_dedicated_router/"
...
def parse_area(rep_dir):
    with open(report_dir + rep_dir + "/Router_Hw.mapped.area.rpt", 'r') as f:
        lines = f.readlines()
        for line in lines:
...
def parse_power(rep_dir):
    with open(report_dir + rep_dir + "/Router_Hw.mapped.power.rpt", 'r') as f:
        lines = f.readlines()
...
for single_rep_dir in all_reports_dir:
    ...
    decomposer = spilt_rep_dir[9]
    temp = spilt_rep_dir[11]
    temp_split = temp.split(".")
    flow_control = temp_split[0]
    single_feature["num_input"] = num_input
    single_feature["num_output"] = num_output
    single_feature["decomposer"] = decomposer
    single_feature["flow_control"] = flow_control
...
output_filename = output_dir + "dedicated_router" + st + ".csv"
